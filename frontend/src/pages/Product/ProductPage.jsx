...
import { Star, Truck, Handshake, Store, ArrowRight, ShoppingCart, Bell, BellRing } from 'lucide-react';
...
export default function ProductPage() {
  ...
  const handleNotify = async () => {
    ...
    try { await api.post(`/shops/products/${id}/notify/`, { phone: notifyPhone }); toast.success('بهت خبر میدیم!'); setShowNotify(false); } catch { toast.error('خطا'); }
  };
              ...
              <div>
                ...
                {isOutOfStock && (
                  <div className="bg-red-50 rounded-2xl p-4 mb-4 border border-red-200">
                    ...
                    {!showNotify ? (
                      <button onClick={() => setShowNotify(true)} className="flex items-center gap-2 text-sm text-pink-600 hover:text-pink-700 font-bold"><Bell size={16} /> در صورت موجود شدن به من خبر بده</button>
                    ) : (
                      <div className="flex gap-2"><input type="tel" value={notifyPhone} onChange={e => setNotifyPhone(e.target.value)} placeholder="۰۹۱۲۳۴۵۶۷۸۹" className="flex-1 px-3 py-2 border rounded-xl text-sm" dir="ltr" /><button onClick={handleNotify} className="bg-pink-600 text-white px-4 py-2 rounded-xl text-sm font-bold"><BellRing size={16} /></button></div>
                    )}
                  </div>
                )}

                {product.stock > 0 && product.stock <= 5 && (
                  <div className="bg-orange-50 rounded-2xl p-3 mb-4 border border-orange-200"><p className="text-orange-700 text-sm">⚠️ فقط {product.stock} عدد دیگر موجود است</p></div>
                )}
              </div>

              <div className="border-t border-gray-100 pt-4 mt-4">
                <div className="flex items-center justify-between mb-4">
                  <div><span className="text-3xl font-extrabold text-gray-900">{Number(product.price).toLocaleString('fa-IR')}</span><span className="text-sm text-gray-500 mr-2">تومان</span></div>
                </div>
                {!isOutOfStock && (
                  <Link to={`/checkout?product=${product.id}`} className="w-full bg-pink-600 hover:bg-pink-700 text-white py-3.5 rounded-2xl font-bold text-lg flex items-center justify-center gap-2 transition-all"><ShoppingCart size={22} /> خرید و پرداخت <ArrowRight size={20} /></Link>
                )}
              </div>
            </div>
          </div>

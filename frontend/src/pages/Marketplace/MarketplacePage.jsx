...
import { Search, SlidersHorizontal, Star, Truck, Handshake, X } from 'lucide-react';
          ...
          {product.color && (
            colorHex ? (
              <div className="flex items-center gap-1.5 mb-2">
                <div style={{ width: "16px", height: "16px", borderRadius: "50%", backgroundColor: colorHex, border: "1px solid #d1d5db", display: "inline-block" }} />
              </div>
            ) : (
              <div className="mb-2">
...
export default function MarketplacePage() {
  ...
  const fetchSuggestions = useCallback(async (q) => {
    ...
    try { const response = await api.get(`/shops/marketplace/suggestions/?q=${q}`); setSuggestions(response.data); setShowSuggestions(true); } catch (error) { console.error(error); }
  }, []);

  useEffect(() => { fetchProducts(); }, [fetchProducts]);
  useEffect(() => { const timer = setTimeout(() => fetchSuggestions(search), 300); return () => clearTimeout(timer); }, [search, fetchSuggestions]);

  const resetFilters = () => { setCondition(''); setMinPrice(''); setMaxPrice(''); setHasCourier(false); setAllowTest(false); setSortBy('newest'); setPage(1); };
      ...
      <div className="max-w-6xl mx-auto px-4 py-4">
        <form onSubmit={(e) => { e.preventDefault(); setShowSuggestions(false); fetchProducts(); }} className="relative mb-4">
          ...
          <input type="text" value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }}
            ...
            placeholder="جستجو..." className="w-full pr-12 pl-4 py-3 bg-white border border-gray-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-pink-500 text-sm" />
          {showSuggestions && (suggestions.products?.length > 0 || suggestions.shops?.length > 0) && (
            <div className="absolute top-full mt-2 w-full bg-white rounded-2xl shadow-xl border p-2 z-50">
              {suggestions.products?.map(p => (
                <button key={p.id} onMouseDown={() => { setSearch(p.title); setShowSuggestions(false); }}
        ...
        <div className="flex items-center gap-2 mb-4 flex-wrap">
          <button onClick={() => setShowFilters(!showFilters)} className="flex items-center gap-1.5 text-sm bg-white border px-3 py-2 rounded-xl hover:border-pink-300"><SlidersHorizontal size={16} />فیلترها</button>
          {SORT_OPTIONS.map(opt => (
            <button key={opt.value} onClick={() => { setSortBy(opt.value); setPage(1); }}
              className={`text-xs px-3 py-2 rounded-xl whitespace-nowrap ${sortBy === opt.value ? 'bg-pink-600 text-white' : 'bg-white border hover:border-pink-300'}`}>{opt.label}</button>
          ))}
        ...
        <AnimatePresence>
          {showFilters && (
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden mb-4">
              <div className="bg-white rounded-2xl p-4 border flex flex-wrap gap-4 items-end">
                <div>
                  <label className="block text-xs text-gray-500 mb-1">وضعیت کالا</label>
                  <div className="flex gap-1 flex-wrap">
                    {CONDITIONS.map(c => (
                      <button key={c.value} onClick={() => { setCondition(c.value); setPage(1); }}
                        className={`text-xs px-2.5 py-1.5 rounded-lg ${condition === c.value ? 'bg-pink-100 text-pink-700' : 'bg-gray-50 hover:bg-gray-100'}`}>{c.label}</button>
                    ))}
                  </div>
                </div>
                <div>
                ...
                <button onClick={resetFilters} className="text-xs text-red-500 hover:text-red-700 flex items-center gap-1"><X size={14} />حذف فیلترها</button>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        {loading ? (
          ...
          <div className="text-center py-20"><div className="text-6xl mb-4">🔍</div><h3 className="text-lg font-bold text-gray-700 mb-2">محصولی یافت نشد</h3></div>
        ) : (
          <>
            ...
            {totalPages > 1 && (
              <div className="flex justify-center gap-2 mt-8">
                {[...Array(totalPages)].map((_, i) => (
                  <button key={i} onClick={() => setPage(i + 1)} className={`w-10 h-10 rounded-xl text-sm font-bold ${page === i + 1 ? 'bg-pink-600 text-white' : 'bg-white text-gray-600 hover:bg-pink-50'}`}>{i + 1}</button>
                ))}
              </div>
            )}
          </>
